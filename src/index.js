...
const anafanafoPromise = import('anafanafo')
let anafanafo;
...
function escapeHTML(input) {
  return input.replace(/[<>&"]/g, s => `&#${s.charCodeAt(0)};`)
}

anafanafoPromise.then(mod => {
  anafanafo = mod.default;
  ...
  document.getElementById('render').onclick = function() {
    ...
    try {
      ...
      diagrams = `<pre>${escapeHTML(''+e)}</pre>`;
    }
    document.getElementById('output').innerHTML = diagrams
  };
});
